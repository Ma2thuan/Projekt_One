...
-----------------------------------------------------------------------------------------------------------------
drop TRIGGER ManagePassbookOnChange

CREATE TRIGGER ManagePassbookOnChange
ON passbook
...
BEGIN
    ...
    DECLARE @InitialDepositAmount FLOAT;
    DECLARE @DepositAmount FLOAT;
    ...
    DECLARE @TimeYears FLOAT;
    DECLARE @CompoundInterest FLOAT;

    -- Check affected rows in INSERTED
    IF EXISTS (SELECT * FROM INSERTED)
    BEGIN
        -- Process inserted or updated rows
        SELECT 
            @SavingsBookID = SavingsBookID,
            @SavingsTypeID = SavingsType,
            @InitialDepositAmount = InitialDepositAmount,
            @DepositAmount = DepositAmount,
            @OpeningDate = OpeningDate
        FROM INSERTED;

        -- Get additional data from SavingsAccountType
        SELECT 
            @InterestRate = InterestRate,
            @Term = Term
        FROM SavingsAccountType
        WHERE SavingsTypeID = @SavingsTypeID;

        -- Determine time in years for interest calculation
        IF @Term IS NULL OR @Term = 0
        BEGIN
            -- Calculate time since opening date for No Term passbooks
            SET @TimeYears = DATEDIFF(DAY, @OpeningDate, @CurrentDate) / 365.0;
            SET @CompoundInterest = @InitialDepositAmount * POWER((1 + @InterestRate / 1), 1 * @TimeYears);
        END
        ELSE
        BEGIN
            -- Calculate maturity date and time in years for passbooks with a term
            SET @MaturityDate = DATEADD(MONTH, @Term, @OpeningDate);
            IF @CurrentDate >= @MaturityDate
            BEGIN
                SET @TimeYears = @Term / 12.0; -- Convert months to years
                SET @CompoundInterest = @DepositAmount * POWER((1 + @InterestRate / 1), 1 * @TimeYears);
            END
        END

        -- Update passbook amounts if interest was calculated
		IF @Term IS NULL OR @Term = 0
		BEGIN
			UPDATE passbook
            SET 
                InitialDepositAmount = InitialDepositAmount + @CompoundInterest ,
                DepositAmount = 0
            WHERE SavingsBookID = @SavingsBookID;
		END

        IF @CompoundInterest IS NOT NULL
        BEGIN
            UPDATE passbook
            SET 
                InitialDepositAmount = InitialDepositAmount + @CompoundInterest - @DepositAmount,
                DepositAmount = 0
            WHERE SavingsBookID = @SavingsBookID;
        END
    END
...
DROP TRIGGER IF EXISTS UpdatePassbookOnDeposit;
GO

CREATE TRIGGER UpdatePassbookOnDeposit
ON SavingsDeposit
AFTER INSERT
AS
BEGIN
    ...
    -- Update the DepositAmount in passbook based on the new deposit
    UPDATE passbook
    SET passbook.DepositAmount = passbook.DepositAmount + i.DepositAmount
    FROM passbook
    INNER JOIN INSERTED i ON passbook.SavingsBookID = i.SavingsBookID;
END;
GO
...
DROP TRIGGER IF EXISTS UpdatePassbookOnWithdrawal;
GO

CREATE TRIGGER UpdatePassbookOnWithdrawal
ON WithdrawalSlip
AFTER INSERT
AS
BEGIN
    ...
    -- Update the DepositAmount in passbook based on the withdrawal
    UPDATE passbook
    SET DepositAmount = DepositAmount - i.WithdrawalAmount
    FROM passbook
    INNER JOIN INSERTED i ON passbook.SavingsBookID = i.SavingsBookID;
    ...
    BEGIN
        ...
        RAISERROR ('Withdrawal amount exceeds the current deposit balance.', 16, 1);
    END
END;
GO
...
DROP TRIGGER trg_SavingsDeposit_AfterInsert
DROP TRIGGER trg_WithdrawalSlip_AfterInsert

-- Trigger for handling deposits in SavingsDeposit

CREATE TRIGGER trg_SavingsDeposit_AfterInsert
ON SavingsDeposit
AFTER INSERT
AS
BEGIN
    ...
    -- Update the InitialDepositAmount in the passbook table
    UPDATE p
    SET p.InitialDepositAmount = p.InitialDepositAmount + i.DepositAmount
    FROM passbook p
    INNER JOIN inserted i ON p.SavingsBookID = i.SavingsBookID;
END;
GO

-- Trigger for handling withdrawals in WithdrawalSlip


CREATE TRIGGER trg_WithdrawalSlip_AfterInsert
ON WithdrawalSlip
AFTER INSERT
AS
BEGIN
    ...
    -- Update the InitialDepositAmount in the passbook table
    UPDATE p
    SET p.InitialDepositAmount = p.InitialDepositAmount - i.WithdrawalAmount
    FROM passbook p
    INNER JOIN inserted i ON p.SavingsBookID = i.SavingsBookID;
END;
GO
